...
public class AddChildUser implements GUIInterface {
	...
	public void initScene() {
		...
		for ( String adult : MainGUI.users.getAllProfiles().keySet() ) {
			if ( MainGUI.users.getProfile(adult) instanceof Adult )
				choices.add(adult);
		}
	...
	public void validateInput() throws NotToBeCoupledException, Exception {
		...
		if ( !choiceOne.getValue().isEmpty() )
			userOne = (Adult) MainGUI.users.getProfile(choiceOne.getValue());
		if ( !choiceTwo.getValue().isEmpty() )
			userTwo = (Adult) MainGUI.users.getProfile(choiceTwo.getValue());
	...
	public String existPartner(String username) {
		if ( ((Adult) MainGUI.users.getProfile(username)).getPartner() != null ) {
			String partnerName = ((Adult) MainGUI.users.getProfile(username)).getPartner().getUsername();
				...
				try {
					...
					if ( Integer.parseInt(childDetails.get(1)) < 3 ) {
						newChild = new YoungChild(childDetails.get(0),
								...
								childDetails.get(4), 
								childDetails.get(5), 
								((Adult) MainGUI.users.getProfile(choiceOne.getValue())), 								
								((Adult) MainGUI.users.getProfile(choiceTwo.getValue())));						
					} else {
					// add a child
					...
					MainGUI.users.addUser(newChild);
					MainGUI.currentUser = MainGUI.users.getProfile(newChild.getUsername()) ;
					...
					if ( MainGUI.links.existingRelationship(choiceOne.getValue(), choiceTwo.getValue()) ) {
						MainGUI.links.changeRelationship(choiceOne.getValue(), choiceTwo.getValue(), "spouse");
					} else {
						MainGUI.links.addRelationship(choiceOne.getValue(), choiceTwo.getValue(), "spouse");
					} 
