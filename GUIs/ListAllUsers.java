...
class ListAllUsers implements EventHandler<ActionEvent>, GUIInterface {
	...
	public void getUsersSubset(String userType) {
		...
		for ( String user : MainGUI.users.getAllProfiles().keySet() ) {
			if (userType.equals("All"))
				users.add(user);
			else if (userType.equals("Adult") && MainGUI.users.getProfile(user) instanceof Adult)
				users.add(user);
			else if (userType.equals("Child") && MainGUI.users.getProfile(user) instanceof Child)
				users.add(user);
			else if (userType.equals("Young Child") && MainGUI.users.getProfile(user) instanceof YoungChild)
				users.add(user);
		}
	...
	public void handle(ActionEvent e) { 		
		...
		showUserBtn.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent arg0) {	
				try {
					User userToShow = MainGUI.users.getProfile(usersList.getSelectionModel().getSelectedItem());
					MainGUI.currentUser = userToShow;
